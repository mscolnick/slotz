...
import { Component } from '../manager/types';
...
export const Slot: React.FC<Props> = (props) => {
  ...
  const elements: React.ReactNode[] = components.flatMap((component, index) => {
    ...
    return children.map((child, index2) => {
      ...
      return React.cloneElement(child, { key: index.toString() + index2.toString(), ...childProps });
    });
  });
  ...
  if (typeof props.children === 'function') {
    ...
    if (React.isValidElement(element) || element === null) {
      ...
      throw new Error('Slot rendered with function must return a valid React Element.');
    }
  } else {
    return <>{elements}</>;
  }
